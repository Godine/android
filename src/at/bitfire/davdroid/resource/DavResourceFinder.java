...
public class DavResourceFinder implements Closeable {
	...
	public void findResources(at.bitfire.davdroid.resource.ServerInfo serverInfo) throws URISyntaxException, DavException, HttpException, IOException {
		// CardDAV
		...
		if (uriAddressBookHomeSet != null) {
			...
			if (checkHomesetCapabilities(homeSetAddressBooks, "addressbook")) {
				...
				List<at.bitfire.davdroid.resource.ServerInfo.ResourceInfo> addressBooks = new LinkedList<>();
				for (WebDavResource resource : possibleAddressBooks)
					if (resource.isAddressBook()) {
						...
						at.bitfire.davdroid.resource.ServerInfo.ResourceInfo info = new at.bitfire.davdroid.resource.ServerInfo.ResourceInfo(
							at.bitfire.davdroid.resource.ServerInfo.ResourceInfo.Type.ADDRESS_BOOK,
							resource.isReadOnly(),
		...
		if (uriCalendarHomeSet != null) {
			...
			if (checkHomesetCapabilities(homeSetCalendars, "calendar-access")) {
				...
				List<at.bitfire.davdroid.resource.ServerInfo.ResourceInfo>
				...
				for (WebDavResource resource : possibleCalendars)
					if (resource.isCalendar()) {
						...
						at.bitfire.davdroid.resource.ServerInfo.ResourceInfo info = new at.bitfire.davdroid.resource.ServerInfo.ResourceInfo(
								at.bitfire.davdroid.resource.ServerInfo.ResourceInfo.Type.CALENDAR,
								resource.isReadOnly(),
								resource.getLocation().toString(),
								resource.getDisplayName(),
								resource.getDescription(), resource.getColor()
						...
						if (isCalendar)
							calendars.add(new at.bitfire.davdroid.resource.ServerInfo.ResourceInfo(info));
						if (isTodoList)
							todoLists.add(new at.bitfire.davdroid.resource.ServerInfo.ResourceInfo(info));
					}
		...
		if (!serverInfo.isCalDAV() && !serverInfo.isCardDAV())
			throw new DavIncapableException("CHANGE ME");//context.getString(R.string.setup_neither_caldav_nor_carddav));
	}
	 ...
	 */
	public URI getInitialContextURL(at.bitfire.davdroid.resource.ServerInfo serverInfo, String serviceName) throws URISyntaxException {
		String	scheme,
				domain;
		int		port = -1;
		String	path = "/";
	 ...
	 */
	WebDavResource getCurrentUserPrincipal(at.bitfire.davdroid.resource.ServerInfo serverInfo, String serviceName) throws URISyntaxException, IOException, NotAuthorizedException {
