...
public class LocalAddressBook extends at.bitfire.davdroid.resource.LocalCollection<at.bitfire.davdroid.resource.Contact> {
	...
	public at.bitfire.davdroid.resource.Contact newResource(long localID, String resourceName, String eTag) {
		at.bitfire.davdroid.resource.Contact c = new at.bitfire.davdroid.resource.Contact(localID, resourceName, eTag);
		c.setVCardVersion(accountSettings.getAddressBookVCardVersion());
		return c;
	}

	public int deleteAllExceptRemoteNames(at.bitfire.davdroid.resource.Resource[] remoteResources) throws at.bitfire.davdroid.resource.LocalStorageException {
		String where;

		if (remoteResources.length != 0) {
			List<String> sqlFileNames = new LinkedList<>();
			for (at.bitfire.davdroid.resource.Resource res : remoteResources)
        ...
        try {
            ...
            throw new at.bitfire.davdroid.resource.LocalStorageException("Couldn't delete contacts locally", e);
        }
    }

	@Override
	public int commit() throws at.bitfire.davdroid.resource.LocalStorageException {
		...
		try {
			...
			throw new at.bitfire.davdroid.resource.LocalStorageException("Couldn't update group names", e);
		}
	...
	@Override
	public void populate(at.bitfire.davdroid.resource.Resource res) throws at.bitfire.davdroid.resource.LocalStorageException {
		at.bitfire.davdroid.resource.Contact c = (at.bitfire.davdroid.resource.Contact)res;

		try {
			@Cleanup EntityIterator iter = RawContacts.newEntityIterator(providerClient.query(
			...
			throw new at.bitfire.davdroid.resource.LocalStorageException(ex);
		}
	}

	private void populateStructuredName(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
	...
	protected void populatePhoneNumber(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
		...
		switch (row.getAsInteger(Phone.TYPE)) {
			...
			case Phone.TYPE_CALLBACK:
				number.addType(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_CALLBACK);
				break;
			case Phone.TYPE_CAR:
				number.addType(TelephoneType.CAR);
				break;
			case Phone.TYPE_COMPANY_MAIN:
				number.addType(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_COMPANY_MAIN);
				break;
			...
			case Phone.TYPE_RADIO:
				number.addType(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_RADIO);
				break;
			...
			case Phone.TYPE_ASSISTANT:
				number.addType(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_ASSISTANT);
				break;
			case Phone.TYPE_MMS:
				number.addType(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_MMS);
				break;
	...
	protected void populateEmailAddress(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
		...
		switch (row.getAsInteger(Email.TYPE)) {
			...
			case Email.TYPE_MOBILE:
				email.addType(at.bitfire.davdroid.resource.Contact.EMAIL_TYPE_MOBILE);
				break;
	...
	protected void populatePhoto(at.bitfire.davdroid.resource.Contact c, ContentValues row) throws RemoteException {
	...
	protected void populateOrganization(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
	...
	protected void populateIMPP(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
	...
	protected void populateNickname(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
	...
	protected void populateNote(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
		c.setNote(row.getAsString(Note.NOTE));
	}

	protected void populatePostalAddress(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
	...
	protected void populateGroupMembership(at.bitfire.davdroid.resource.Contact c, ContentValues row) throws RemoteException {
	...
	protected void populateURL(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
		c.getURLs().add(row.getAsString(Website.URL));
	}

	protected void populateEvent(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
	...
	protected void populateRelation(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
		...
		switch (row.getAsInteger(Relation.TYPE)) {
			...
			case Relation.TYPE_BROTHER:
				...
				types.add(at.bitfire.davdroid.resource.Contact.RELATED_TYPE_BROTHER);
				break;
			...
			case Relation.TYPE_FATHER:
				types.add(at.bitfire.davdroid.resource.Contact.RELATED_TYPE_FATHER);
				break;
	...
	protected void populateSipAddress(at.bitfire.davdroid.resource.Contact c, ContentValues row) {
		try {
	...
	@Override
	protected Builder buildEntry(Builder builder, at.bitfire.davdroid.resource.Resource resource, boolean update) {
		at.bitfire.davdroid.resource.Contact contact = (at.bitfire.davdroid.resource.Contact)resource;
	...
	@Override
	protected void addDataRows(at.bitfire.davdroid.resource.Resource resource, long localID, int backrefIdx) {
		at.bitfire.davdroid.resource.Contact contact = (at.bitfire.davdroid.resource.Contact)resource;
	...
	@Override
	protected void removeDataRows(at.bitfire.davdroid.resource.Resource resource) {
	...
	protected Builder buildStructuredName(Builder builder, at.bitfire.davdroid.resource.Contact contact) {
		return builder
	...
	protected Builder buildPhoneNumber(Builder builder, Telephone number) {
		...
		} else if (types.contains(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_CALLBACK)) {
			typeCode = Phone.TYPE_CALLBACK;
		} else if (types.contains(TelephoneType.CAR)) {
			typeCode = Phone.TYPE_CAR;
		} else if (types.contains(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_COMPANY_MAIN)) {
			typeCode = Phone.TYPE_COMPANY_MAIN;
		} else if (types.contains(TelephoneType.ISDN)) {
			typeCode = Phone.TYPE_ISDN;
		} else if (types.contains(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_RADIO)) {
		...
		} else if (types.contains(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_ASSISTANT)) {
			typeCode = Phone.TYPE_ASSISTANT;
		} else if (types.contains(at.bitfire.davdroid.resource.Contact.PHONE_TYPE_MMS)) {
	...
	protected Builder buildEmail(Builder builder, ezvcard.property.Email email) {
		...
		for (EmailType type : email.getTypes())
			...
			else if (type == at.bitfire.davdroid.resource.Contact.EMAIL_TYPE_MOBILE)
	...
	protected Builder buildOrganization(Builder builder, at.bitfire.davdroid.resource.Contact contact) {
