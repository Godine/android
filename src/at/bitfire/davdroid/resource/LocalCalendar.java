 ...
 */
public class LocalCalendar extends at.bitfire.davdroid.resource.LocalCollection<at.bitfire.davdroid.resource.Event> {
	...
	public static Uri create(Account account, ContentResolver resolver, ServerInfo.ResourceInfo info) throws at.bitfire.davdroid.resource.LocalStorageException {
		@Cleanup("release") final ContentProviderClient client = resolver.acquireContentProviderClient(CalendarContract.AUTHORITY);
		if (client == null)
			throw new at.bitfire.davdroid.resource.LocalStorageException("No Calendar Provider found (Calendar app disabled?)");
		...
		try {
			...
			throw new at.bitfire.davdroid.resource.LocalStorageException(e);
		}
	}
	...
	@Override
	public String getCTag() throws at.bitfire.davdroid.resource.LocalStorageException {
		try {
			...
			if (c != null && c.moveToFirst())
				...
				throw new at.bitfire.davdroid.resource.LocalStorageException("Couldn't query calendar CTag");
		} catch(RemoteException e) {
			throw new at.bitfire.davdroid.resource.LocalStorageException(e);
		}
	}
	...
	@Override
	public void setCTag(String cTag) throws at.bitfire.davdroid.resource.LocalStorageException {
	...
	@Override
	public long[] findUpdated() throws at.bitfire.davdroid.resource.LocalStorageException {
	...
	public at.bitfire.davdroid.resource.Event newResource(long localID, String resourceName, String eTag) {
		return new at.bitfire.davdroid.resource.Event(localID, resourceName, eTag);
	}
	
	public int deleteAllExceptRemoteNames(at.bitfire.davdroid.resource.Resource[] remoteResources) throws LocalStorageException {
	...
	@Override
	public void delete(at.bitfire.davdroid.resource.Resource resource) {
	...
	@Override
	public void clearDirty(at.bitfire.davdroid.resource.Resource resource) {
	...
	@Override
	public void populate(at.bitfire.davdroid.resource.Resource resource) throws LocalStorageException {
		at.bitfire.davdroid.resource.Event event = (at.bitfire.davdroid.resource.Event)resource;
		...
		try {
			...
			throw new at.bitfire.davdroid.resource.LocalStorageException("Couldn't process locally stored event", ex);
		}
	}

	protected void populateEvent(at.bitfire.davdroid.resource.Event e, ContentValues values) {
	...
	void populateExceptions(at.bitfire.davdroid.resource.Event e) throws RemoteException {
		...
		while (c != null && c.moveToNext()) {
			...
			try {
				at.bitfire.davdroid.resource.Event exception = new at.bitfire.davdroid.resource.Event(exceptionId, exceptionRemoteName, null);
				populate(exception);
				e.getExceptions().add(exception);
			} catch (at.bitfire.davdroid.resource.LocalStorageException ex) {
				Log.e(TAG, "Couldn't find exception details, ignoring");
			}
		}
	}

	void populateAttendee(at.bitfire.davdroid.resource.Event event, ContentValues values) {
		try {
	...
	void populateReminder(at.bitfire.davdroid.resource.Event event, ContentValues row) {
	...
	protected Builder buildEntry(Builder builder, at.bitfire.davdroid.resource.Resource resource, boolean update) {
		final at.bitfire.davdroid.resource.Event event = (at.bitfire.davdroid.resource.Event)resource;
	...
	protected void addDataRows(at.bitfire.davdroid.resource.Resource resource, long localID, int backrefIdx) {
		...
		// add exceptions
		for (at.bitfire.davdroid.resource.Event exception : event.getExceptions())
	...
	protected Builder buildException(Builder builder, at.bitfire.davdroid.resource.Event master, at.bitfire.davdroid.resource.Event exception) {
