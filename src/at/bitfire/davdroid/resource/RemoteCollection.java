 ...
 */
public abstract class RemoteCollection<T extends at.bitfire.davdroid.resource.Resource> {
	 ...
	 */
	public at.bitfire.davdroid.resource.Resource[] getMemberETags() throws URISyntaxException, IOException, DavException, HttpException {
		...
		return resources.toArray(new at.bitfire.davdroid.resource.Resource[resources.size()]);

	}


	@SuppressWarnings("unchecked")
	public at.bitfire.davdroid.resource.Resource[] multiGet(Resource[] resources) throws URISyntaxException, IOException, DavException, HttpException {
		try {
			...
			return foundResources.toArray(new at.bitfire.davdroid.resource.Resource[foundResources.size()]);
		} catch (at.bitfire.davdroid.resource.InvalidResourceException e) {
			Log.e(TAG, "Couldn't parse entity from GET", e);
		}

		return new at.bitfire.davdroid.resource.Resource[0];
	}
	
	
	/* internal member operations */

	public at.bitfire.davdroid.resource.Resource get(at.bitfire.davdroid.resource.Resource resource) throws URISyntaxException, IOException, HttpException, DavException, at.bitfire.davdroid.resource.InvalidResourceException {
		...
		try {
			...
			throw new at.bitfire.davdroid.resource.InvalidResourceException(e);
		}
		return resource;
	}

	// returns ETag of the created resource, if returned by server
	public String add(at.bitfire.davdroid.resource.Resource res) throws URISyntaxException, IOException, HttpException {
	...
	public void delete(at.bitfire.davdroid.resource.Resource res) throws URISyntaxException, IOException, HttpException {
		WebDavResource member = new WebDavResource(collection, res.getName(), res.getETag());
		member.delete();
	...
	public String update(at.bitfire.davdroid.resource.Resource res) throws URISyntaxException, IOException, HttpException {
		WebDavResource member = new WebDavResource(collection, res.getName(), res.getETag());
		member.setContentType(res.getMimeType());
	...
	at.bitfire.davdroid.resource.Resource.AssetDownloader getDownloader() {
		return new at.bitfire.davdroid.resource.Resource.AssetDownloader() {
            @Override
